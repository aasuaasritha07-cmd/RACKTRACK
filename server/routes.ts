...
// Note: This server uses EmailJS on the client-side for email delivery. Server-side Resend usage removed.
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Server-side emailing is disabled. Use EmailJS client-side for immediate sends.
  ...
  app.post("/api/contact", async (req, res) => {
    try {
      ...
      console.log("[Contact] Will send contact email to recipients:", recipients);
      ...
      // Note: Server will not send emails; EmailJS is used client-side. Keep fields for optional future use.
      ...
      // Do not attempt a server-side send. Return a default sendResult.
      const sendResult: any = { hasErrors: false, details: [] };
      const anySendErrors = false;

      // Update contact delivery metadata
      try {
        const lastError = sendResult.details.length > 0 ? sendResult.details.map((d: any) => `${d.to}: ${((d as any).body ?? (d as any).error ?? (d as any).statusText ?? (d as any).status ?? 'unknown')}`).join("; ") : undefined;
        await storage.updateContact((contact as any).id, {
          emailAttempts: ((contact as any).emailAttempts || 0) + 1,
          emailLastAttemptAt: new Date() as any,
          emailLastError: undefined as any,
          emailSent: false,
        } as any);
      } catch (e) {
        console.warn("[Contact] Failed to persist contact delivery metadata:", e);
      }

      // Return success: the contact was persisted, EmailJS should handle delivery
      res.status(201).json({ success: true, emailSent: false, anySendErrors: false, contact });
    } catch (error) {
  ...
  // Admin: list unsent contacts (protected by CONTACT_TEST_KEY header)
  app.get("/api/admin/contacts/unsent", async (req, res) => {
    try {
      const key = req.headers["x-test-key"] as string | undefined;
      if (!key || key !== process.env.CONTACT_TEST_KEY) {
        return res.status(401).json({ success: false, message: "Invalid admin key" });
      }
      const all = await storage.getAllContacts();
      const unsent = all.filter((c) => !((c as any).emailSent));
      res.json({ success: true, contacts: unsent });
    } catch (err) {
      console.error("/api/admin/contacts/unsent error:", err);
